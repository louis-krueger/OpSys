 ...
 */
syscall	create(void *funcaddr, ulong ssize,
			   char *name, ulong nargs, ...)
{
	...
	*--saddr = (ulong)ppcb->stkbase;
	...
	if (nargs < 4)                       /* If more than 4 args,         */
	{ pads = 0; }                        /*  pad record size to multiple */
	else                                 /*  of native memory transfer   */
	{ pads = nargs - 4 + (4 - (nargs % 4)); }

	for (i = 0; i < pads; i++) { *--saddr = 0; }
                                             /* Reserve space for extra args */
